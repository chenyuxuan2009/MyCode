...
void solve()
{
	...
	for(int i=1;i<n;i++)
	{
		if(abs(a[i]-a[i+1])<=k&&f1[i]==1)f1[i+1]=1;
		if(abs(b[i]-b[i+1])<=k&&f2[i]==1)f2[i+1]=1;
		if(abs(a[i]-b[i+1])<=k&&f1[i]==1)f2[i+1]=1;
		if(abs(b[i]-a[i+1])<=k&&f2[i]==1)f1[i+1]=1;
	}
	for(int i=1;i<=n;i++)
	{
		if(f1[i]!=1&&f2[i]!=1)f=1;
	}
	puts(f==1?"No":"Yes");
}
int main()
